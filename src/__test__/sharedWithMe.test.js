...
test("Se renderizan bien los componentes de SharedWithMe", () => {
  ...
  expect(getByTestId("textoCompartidoConmigo")).toHaveTextContent(
    "En esta sección puedes ver los detalles de las rutas que te han compartido tus amigos, junto con sus comentarios y fotos."
  );
});
...
test("No nos han compartido ninguna ruta, por tanto, se muestra la alerta", async () => {
  ...
  const { getByTestId } = render(
    <SharedWithMe
      handleLoaded={jest.fn()}
      getRutasCompartidasConmigo={jest.fn(() => {
        return [];
      })}
      subirFicheroARuta={rutaService.subirFicheroARuta}
      obtenerFicherosRuta={rutaService.obtenerFicherosRuta}
  ...
  let alertaNoRutasCompartidas = await waitForElement(() =>
    getByTestId("alertaNoRutasCompartidas")
  );
  expect(alertaNoRutasCompartidas).toHaveTextContent(
